...
export async function analyzeVideo(id: string, signal?: AbortSignal, onProcess?: (child: ChildProcess) => void) {
  ...
  const raw = await new Promise<RawInfo>((resolve, reject) => {
    ...
    const child = spawn(binary, ['--ignore-config','--no-playlist','--skip-download','--dump-single-json','--no-progress','--no-warnings','--no-cache-dir','--js-runtimes',`node:${process.execPath}`,'--remote-components','ejs:github','--extractor-args','youtube:player_client=android_vr,web_safari','--socket-timeout','15','--retries','1','--extractor-retries','1','--',`https://www.youtube.com/watch?v=${id}`], { windowsHide: true, detached: process.platform !== 'win32' });
