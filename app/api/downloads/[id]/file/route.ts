...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const ext = job.file.endsWith('.webm') ? 'webm' : 'mp4';
  ...
  return new Response(Readable.toWeb(stream) as ReadableStream, { headers: {
    'Content-Type': `video/${ext}`, 'Content-Length': String(job.size),
    'Content-Disposition': `attachment; filename="video.${ext}"; filename*=UTF-8''${encodeURIComponent(name).replace(/['()*]/g, c => '%' + c.charCodeAt(0).toString(16))}`,
